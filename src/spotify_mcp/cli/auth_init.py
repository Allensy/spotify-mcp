...
def auto_auth(settings) -> bool:
        ...
        try:
            ...
            if redirect_response:
                try:
                    ...
                    print(f"\n✗ Failed to parse redirect URL: {e}", file=sys.stderr)
                    return False
            else:
    ...
    if CallbackHandler.auth_code:
        ...
        try:
            token_info = auth_manager.get_access_token(CallbackHandler.auth_code, as_dict=True)
            if token_info:
...
def main() -> None:
    ...
    if use_auto:
        ...
        print("💡 Tip: Use --auto for automatic browser-based authentication\n")
