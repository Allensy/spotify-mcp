...
class TestAuthInitModule:
    ...
    def test_auth_init_imports(self):
        ...
        from spotify_mcp.cli.auth_init import main, auto_auth, manual_auth

        assert callable(main)
        assert callable(auto_auth)
        assert callable(manual_auth)
...
class TestAutoAuthFlow:
    ...
    def test_auto_auth_starts_server(
        self, mock_oauth, mock_http_server, mock_webbrowser
    ):
        ...
        with patch("spotify_mcp.cli.auth_init.HTTPServer"):
            with patch("spotify_mcp.cli.auth_init.SpotifyOAuth") as mock_oauth:
                ...
                mock_input.return_value = (
                    "http://127.0.0.1:8888/callback?code=test_code"
                )
...
class TestManualAuthFlow:
    ...
    def test_manual_auth_prompts_for_url(self, mock_oauth, mock_input):
        ...
        assert result is True
        mock_input.assert_called_once()
        mock_auth.parse_response_code.assert_called_once_with(
            "http://127.0.0.1:8888/callback?code=test_code"
        )
